...
class CFPT(nn.Module):
    def __init__(self,
                 ...
                 mid_channels=256, alpha=1, beta=1, num_heads=4, mlp_ratio=2,
                 reduction=4):
        super(CFPT, self).__init__()
        self.in_channels = in_channels
        self.num_layers = len(in_channels)
        self.convs = nn.ModuleList([BasicConv(c,mid_channels,1) for c in in_channels])
        ...
        self.out_convs = nn.ModuleList([BasicConv(mid_channels,c,1) for c in in_channels])

    # @auto_fp16()
    def forward(self, inputs):
        x_list = [self.convs[i](inputs[i]) for i in range(len(inputs))]
        x_list = self.cross_layer_channel_attn(x_list)
        x_list = self.cross_layer_spatial_attn(x_list)
        x_list = [self.out_convs[i](x_list[i]) for i in range(len(x_list))]
        x_list = [(i + j).type(inputs[0].dtype) for step, (i, j) in enumerate(zip(inputs, x_list))]
        return tuple(x_list)

if __name__ == '__main__':
    model = CFPT(in_channels=[16,32,64,128],mid_channels=32)
    x1 = torch.rand(2, 16, 32, 32)
    x2 = torch.rand(2, 32, 16, 16)
    x3 = torch.rand(2, 64, 8, 8)
    x4 = torch.rand(2, 128, 4, 4)
    o = model([x1,x2,x3,x4])
    for i in o:
        print(i.shape)
    pass